...
# =============================================================================
# Resumable runner for COUNT queries across all 51 CP-free variable orders.
#
# Mirrors run_all_51vo.sh but uses the _count query directories and
# SELECT SUM(1) queries (no GROUP BY, no aggregation).
#
# Key features:
...
#   SCALES        - space-separated list of scale factors (default: "sf1")
...
PROGRESS_FILE="${OUT_DIR}/progress_51vo_count.txt"
...
print_summary() {
  echo ""
  echo "============================================================"
  echo "PROGRESS SUMMARY — COUNT QUERIES ($(date '+%Y-%m-%d %H:%M:%S'))"
  echo "============================================================"
...
for Q in $QUERIES; do
  VO_DIR="tods26-experiments/queries/tpch_query_${Q}_count/variable_orders"
  ...
  done < <(find "$VO_DIR" -maxdepth 1 -type f -name "tpch_q${Q}cnt_*.txt" 2>/dev/null | sort)
done

TOTAL=${#MANIFEST[@]}

if [[ "$TOTAL" -eq 0 ]]; then
  echo "ERROR: No count experiments found. Run generate_assets.py in the _count directories first."
  exit 1
fi
...
for entry in "${MANIFEST[@]}"; do
  ...
  log_out="${OUT_DIR}/q${Q}count_${vo_name}_${scale}_${mode}_pred_${pred}.csv"
  if [[ -s "$log_out" ]]; then
...
echo "============================================================"
echo "F-IVM COUNT Query Runner — All 51 CP-free Variable Orders"
echo "============================================================"
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  for entry in "${MANIFEST[@]}"; do
    ...
    log_out="${OUT_DIR}/q${Q}count_${vo_name}_${scale}_${mode}_pred_${pred}.csv"
    if [[ -s "$log_out" ]]; then
      echo "  [SKIP] Q${Q}cnt ${vo_name} ${scale} ${mode} pred:${pred}"
    else
      echo "  [TODO] Q${Q}cnt ${vo_name} ${scale} ${mode} pred:${pred}"
    fi
  done
  exit 0
fi
...
for entry in "${MANIFEST[@]}"; do
  ...
  sql_file="tods26-experiments/queries/tpch_query_${Q}_count/sql_files/${vo_name}_${scale}_${mode}_pred_${pred}.sql"
  ...
  if [[ -s "$log_out" ]]; then
    ...
    if [[ $((SKIPPED % 50)) -eq 1 ]] || [[ "$SKIPPED" -eq "$DONE_PREV" ]]; then
      echo "[${IDX}/${TOTAL}] Q${Q}cnt ${vo_name} ${scale} ${mode} pred:${pred} — SKIP (${SKIPPED}/${DONE_PREV} skipped)"
    fi
    continue
  fi

  echo -n "[${IDX}/${TOTAL}] Q${Q}cnt ${vo_name} ${scale} ${mode} pred:${pred} ... "
  START_TIME=$(date +%s)
...
echo ""
echo "ALL COUNT EXPERIMENTS PROCESSED."
