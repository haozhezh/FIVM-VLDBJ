IMPORT DTREE FROM FILE '../variable_orders/tpch_q9cnt_vo15_supp_root_order_nation_bushy.txt';
...
CREATE STREAM PART (
  ...
  FROM FILE './datasets/updates_sf0p1_b10000_dynamic/part.csv'
  LINE DELIMITED CSV (delimiter := '|', predefined_batches := 'true');
...
CREATE STREAM PARTSUPP (
  ...
  FROM FILE './datasets/updates_sf0p1_b10000_dynamic/partsupp.csv'
  LINE DELIMITED CSV (delimiter := '|', predefined_batches := 'true');
...
CREATE STREAM SUPPLIER (
  ...
  FROM FILE './datasets/updates_sf0p1_b10000_dynamic/supplier.csv'
  LINE DELIMITED CSV (delimiter := '|', predefined_batches := 'true');
